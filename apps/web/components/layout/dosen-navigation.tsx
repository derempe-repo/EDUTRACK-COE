...
const navigationItems = [
  {
    ...
    label: "Dashboard",
    isActive: (pathname: string) => pathname === "/dosen/dashboard",
  },
  {
    href: "/dosen/dashboard#kelas-diampu",
    icon: BookOpen,
    label: "Kelas Diampu",
    isActive: (pathname: string) => pathname.startsWith("/dosen/classes"),
  },
  {
    href: "/dosen/dashboard#antrean-review",
    icon: ClipboardCheck,
    label: "Review Masuk",
    isActive: () => false,
  },
...
export function DosenDesktopNavigation({ unreadNotificationCount }: DosenNavigationProps) {
  const pathname = usePathname();

  return (
    <nav aria-label="Navigasi dosen" className="space-y-1 px-4">
        ...
        return (
          ...
          >
            <span className="relative">
              ...
              {item.label === "Notifikasi" ? <UnreadBadge count={unreadNotificationCount} /> : null}
            </span>
...
export function DosenMobileNavigation({ unreadNotificationCount }: DosenNavigationProps) {
  const pathname = usePathname();
  const mobileItems = navigationItems.filter((item) => item.label !== "Review Masuk");

  return (
    <nav
      aria-label="Navigasi dosen mobile"
      ...
      {mobileItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            <span>{item.label === "Kelas Diampu" ? "Kelas" : item.label}</span>
          </Link>
        );
      })}
